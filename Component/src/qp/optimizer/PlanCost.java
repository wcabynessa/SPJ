...
public class PlanCost{
    ...
    protected int getStatistics(Join node){
		...
		//cannot join without at least 3 buffer
		if (numbuff < 3) {
			isFeasible = false;
			return outtuples;
		}
		...
		switch(joinType){
			...
			case JoinType.BLOCKNESTED:
				int numpass = (int) Math.ceil((double) leftpages/(double) numbuff);
			    joincost = leftpages  + numpass * rightpages;
			    break;

			case JoinType.SORTMERGE:
				int numPass;
				numPass = 1 + (int) Math.ceil(Math.log((double) leftpages/ (double) numbuff) / Math.log((double) numbuff - 2));
			    int leftSortCost = numPass * leftpages;
				numPass = 1 + (int) Math.ceil(Math.log((double) rightpages/ (double) numbuff) / Math.log((double) numbuff - 2));
				int rightSortCost = numPass * rightpages;
